...
EXISTING_PIDS="$(pgrep -f '[b]asicsr/train.py.*configs/stage1_report.yaml' || true)"
if [[ -n "${EXISTING_PIDS}" ]]; then
    echo "Stage 1 training is already running (PID: ${EXISTING_PIDS//$'\n'/, })." >&2
    echo "Stop the existing process before starting a rewind." >&2
    exit 1
fi
...
TRAIN_COMMAND=(
    ...
    -opt configs/stage1_report.yaml
    --auto_resume
)
...
echo "Stage 1 resume started in tmux session: ${SESSION_NAME}"
echo "Checkpoint: ${RESUME_ITER:-latest complete pair}"
echo "W&B mode: ${WANDB_RESUME_MODE:-resume (from configs/stage1_report.yaml)}"
