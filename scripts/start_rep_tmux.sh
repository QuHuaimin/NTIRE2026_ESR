...
EXISTING_PIDS="$(pgrep -f "[b]asicsr/train.py.*${CONFIG}" || true)"
if [[ -n "${EXISTING_PIDS}" ]]; then
    echo "${STAGE} REP training is already running (PID: ${EXISTING_PIDS//$'\n'/, })." >&2
    exit 1
fi
...
echo "${STAGE} REP training started in tmux session: ${SESSION_NAME}"
echo "Config: ${CONFIG}"
echo "Checkpoint: ${RESUME_ITER:-latest complete pair, or new training}"
echo "Attach: tmux attach -t ${SESSION_NAME}"
echo "Detach from another terminal: tmux detach-client -s ${SESSION_NAME}"
